...
//Card 1
String my_id;
String secret_key [] = {"genesis","exodus"};
String random_id;
  ...
  if (debouncer.fell()) {
    ...
    for (int i = 0; i <= 1; i++) {
      ...
      if (WiFi.status() == WL_CONNECTED) { //Check WiFi connection status
        ...
        JSONencoder["my_id"] = available_ids[i % 2]; // card id
        JSONencoder["secret_key"] = secret_key[i % 2]; // password
        JSONencoder["other_id"] = available_ids[(i+1) % 2];//getRandomId();

        Serial.println(available_ids[i % 2]);
        Serial.println(available_ids[(i+1) % 2]);
